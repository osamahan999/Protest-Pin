...
describe("Testing Events Functionalities", () => {
    ...
    const event_id: number = 54

    //Expected Output:  ✓ Creating an event (381ms)
    ...
    //Expected Output:     ✓ Join the event (180ms)
    it("Join the event", async () => {
        const join_event = await event.joinEvent(user_id, event_id);

        expect(join_event.message).to.equal("Successfully joined event!")
    })

    //Expected Output:     ✓ Get events (182ms)
    it("Get events", async () => {
        const events = await event.getEvents();
        expect(events.http_id).to.equal(200)
    })
    //Expected Output:     ✓ Get specific user's events (176ms)
